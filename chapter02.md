...
饥荒的代码有一个非常突出的设计风格：**逻辑和数据分离**。战斗系统的代码不写死"斧头伤害 27"，而是读取 `TUNING.AXE_DAMAGE`；制作系统不在代码中列出"1 树枝 + 1 燧石 = 斧头"，而是从 `recipes.lua` 的数据表读取。这种风格叫做"数据驱动设计"。

### 2.13.1 什么是数据驱动设计

> **给新手的类比**：想象一个餐厅。
> - **硬编码**：厨师把菜谱背在脑子里，改菜只能换厨师
> - **数据驱动**：菜谱写在菜单上，厨师按菜单做菜，改菜只需要改菜单

在游戏开发中：

```lua
-- 硬编码方式（不好）
function Axe:OnEquip()
    self.damage = 27.2
    self.max_uses = 100
end
...
-- 数据驱动方式（饥荒的做法）
function Axe:OnEquip()
    self.damage = TUNING.AXE_DAMAGE         -- 从数据表读取
    self.max_uses = TUNING.AXE_USES         -- 改数值只需改数据表
end
```

数据驱动的好处：
1. **策划不需要改代码**——只需调整数据表就能平衡游戏
2. **Mod 友好**——修改一个数值不需要复制整个文件
3. **集中管理**——所有数值在一个地方，一目了然
4. **支持覆盖和修改器**——可以动态叠加修改

### 2.13.2 tuning.lua——游戏的"数值大脑"

`tuning.lua` 包含了饥荒中**几乎所有可调节的游戏数值**，约 9000 行。
...
function Tune(overrides)
    -- 基础时间常量
    local seg_time = 30                    -- 一个时段 30 秒
    local total_day_time = seg_time * 16   -- 一天 480 秒
    ...
    TUNING = {
        ...
        -- 食物
        CALORIES_PER_DAY = 75,
        PERISH_ONE_DAY = 1 * total_day_time,
        PERISH_TWO_DAY = 2 * total_day_time,
...
注意 `TUNING` 表中的数值**相互引用**——斧头伤害是基于 `wilson_attack * 0.8` 计算的，这意味着修改 `wilson_attack` 会自动调整所有以它为基准的武器伤害。

#### TUNING 的修改器系统

`tuning.lua` 提供了一个修改器（Modifier）系统，允许在不直接覆盖值的情况下动态调整数值：
...
function AddTuningModifier(tuning_var, fn, tuning_value)
    if not TUNING_MODIFIERS[tuning_var] and fn then
        ...
        TUNING[tuning_var] = nil   -- 原值被移除
    end
end
```

当访问被修改器保护的 TUNING 值时，会通过修改器函数动态计算。这让世界设置（如"更多/更少蜘蛛"）可以影响 TUNING 值而不冲突。
...
-- 方法 1：直接覆盖（简单粗暴，但可能被后加载的 Mod 覆盖）
TUNING.WILSON_HEALTH = 200

-- 方法 2：修改器（更安全，支持多个 Mod 叠加）
AddTuningModifier("WILSON_HEALTH", function(original)
    return original * 1.5   -- 血量变为原来的 1.5 倍
end)

-- 方法 3：读取并微调
TUNING.AXE_DAMAGE = TUNING.AXE_DAMAGE + 10  -- 增加 10 点
```

### 2.13.3 recipes.lua——制作配方数据表

`recipes.lua` 定义了游戏中所有可制作物品的配方。它是纯数据——只调用 `Recipe2()` 函数声明每个配方的材料、科技需求和其他属性。

#### Recipe2 的参数

```lua
Recipe2(name, ingredients, tech, options)
```

| 参数 | 说明 |
|---|---|
| `name` | 配方名称（通常等于产物的 prefab 名） |
| `ingredients` | 材料列表 `{Ingredient("type", amount), ...}` |
| `tech` | 科技需求 `TECH.XXX` |
| `options` | 可选参数表（builder_tag、placer、numtogive 等） |
...
Recipe2("lighter", {
    ...
    builder_tag = "pyromaniac"   -- 只有薇洛能制作
})
```
...
Ingredient = Class(function(self, ingredienttype, amount, atlas, deconstruct, imageoverride)
    self.type = ingredienttype    -- 材料的 prefab 名
    self.amount = amount          -- 需要的数量
    self.atlas = atlas            -- 图标图集（可选）
    self.image = imageoverride    -- 图标覆盖（可选）
    self.deconstruct = deconstruct -- 拆解时是否返还
end)
```

还有特殊的**角色材料**（消耗角色的属性而非物品）：
...
-- Recipe 构造函数中
AllRecipes[self.name] = self
```

#### Mod 中添加配方

```lua
-- 在 modmain.lua 中使用 modutil 提供的 API
local myrecipe = AddRecipe2("mymod_sword",
    {Ingredient("goldnugget", 3), Ingredient("twigs", 2)},
    TECH.SCIENCE_ONE,
    {atlas = "images/inventoryimages/mymod_sword.xml"}
)
```

### 2.13.4 containers.lua——容器的 UI 数据表

`containers.lua` 定义了每种容器（箱子、背包、冰箱、切斯特等）的**UI 布局和交互参数**——有多少格、格子怎么排列、显示什么动画。
...
params.backpack = {
    widget = {
        ...
        pos = Vector3(-5, -80, 0),  -- 窗口位置
    },
    issidewidget = true,            -- 是否显示在侧面
    type = "pack",                  -- 容器类型
    openlimit = 1,                  -- 同时只能开一个
}
...
当一个容器组件被初始化时，调用 `widgetsetup` 把数据注入：
...
function containers.widgetsetup(container, prefab, data)
    ...
    if t ~= nil then
        for k, v in pairs(t) do
            container[k] = v   -- 把所有参数复制到容器组件上
        end
        container:SetNumSlots(#container.widget.slotpos or 0)
    end
end
```

这意味着容器的逻辑代码（`components/container.lua`）完全不关心"几格、怎么排列"——它只负责"存取物品"的逻辑，布局数据从 `containers.lua` 注入。
...
for y = 2, 0, -1 do
    for x = 0, 2 do
        table.insert(params.mymod_chest.widget.slotpos,
            Vector3(-162 + x * 75, -75 * y + 75, 0))
    end
end
```

### 2.13.5 PROTOTYPER_DEFS——制作站定义

`recipes.lua` 文件顶部还定义了所有**制作站**（科学机器、炼金引擎、远古祭坛等）的元数据：
...
PROTOTYPER_DEFS = {
    ...
    ancient_altar = {   -- 远古祭坛
        ...
        is_crafting_station = true,  -- 必须站在旁边才能制作
...
`is_crafting_station = false` 意味着解锁后可以随时制作（如科学机器），`true` 意味着必须站在旁边（如远古祭坛）。

### 2.13.6 数据驱动设计的三层架构

把这些文件放在一起看，饥荒的数据驱动设计形成了清晰的三层架构：
...
local function fn()
    ...
    -- 读取数据层
    inst:AddComponent("weapon")
    inst.components.weapon:SetDamage(TUNING.AXE_DAMAGE)    -- ← tuning.lua

    inst:AddComponent("finiteuses")
    inst.components.finiteuses:SetMaxUses(TUNING.AXE_USES) -- ← tuning.lua
...
-- 数据层 strings.lua
STRINGS.NAMES.AXE = "Axe"
STRINGS.RECIPE_DESC.AXE = "Keep your eye on the birdie."
```
...
| 文件 | 管理什么数据 |
|---|---|
| `tuning.lua` | 所有数值常量（伤害、血量、时间等） |
| `recipes.lua` | 制作配方（材料、科技、制作站） |
| `containers.lua` | 容器 UI 布局（格子位置、动画） |
| `strings.lua` | 所有文本（名称、描述、台词） |
| `constants.lua` | 枚举常量（装备槽位、食物类型、动作） |
| `cooking.lua` | 烹饪配方（食材→成品的规则） |
| `techtree.lua` | 科技树定义 |
| `actions.lua` | 动作定义（砍、挖、攻击等） |
| `prefabskin.lua` | 皮肤映射数据 |

### 2.13.8 Mod 开发中的数据驱动实践

遵循数据驱动设计，你的 Mod 也应该把数据和逻辑分离：

```lua
-- 不好的做法：数值硬编码在 prefab 中
local function fn()
    inst:AddComponent("weapon")
    inst.components.weapon:SetDamage(50)  -- 硬编码
end

-- 好的做法：数值放在 TUNING 中
TUNING.MYMOD_SWORD_DAMAGE = 50
TUNING.MYMOD_SWORD_USES = 150

local function fn()
    inst:AddComponent("weapon")
    inst.components.weapon:SetDamage(TUNING.MYMOD_SWORD_DAMAGE)
    inst:AddComponent("finiteuses")
    inst.components.finiteuses:SetMaxUses(TUNING.MYMOD_SWORD_USES)
end
```

好处：其他 Mod 可以通过修改 `TUNING.MYMOD_SWORD_DAMAGE` 来调整你的武器，而不需要覆盖你的整个 prefab 文件。

---

> **本节小结**
> - **数据驱动设计**：逻辑和数据分离，改数值不需要改代码
> - `tuning.lua`：9000+ 行的数值常量表，支持修改器系统（`AddTuningModifier`）
> - `recipes.lua`：制作配方的纯数据声明，通过 `Recipe2()` 定义材料、科技需求等
> - `containers.lua`：容器 UI 布局的纯数据表，通过 `widgetsetup` 注入到容器组件
> - 饥荒的三层架构：数据层（tuning/recipes/strings）→ 逻辑层（components/stategraphs）→ 实体层（prefabs）
> - Mod 开发中应遵循同样的模式：把数值放在 TUNING 中，让其他 Mod 也能方便地修改
